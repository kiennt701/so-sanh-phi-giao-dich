...
import { ArrowUpRight, Award, Building2, Check, Plus, Sparkles, TrendingDown, ChevronRight, Shield, Zap } from 'lucide-react';
...
export default function SecuritiesCards({
  ...
  return (
    <section className="broker-cards-grid grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {companies.map((company) => {
        ...
        return (
          <article
            key={company.id}
            className={`broker-card flex flex-col justify-between rounded-2xl p-4 transition-all group ${
              isBsc
            ...
            <header className="broker-card__header">
              ...
              {isBsc && (
                <div className="mb-2.5 -mt-1 flex items-center justify-between rounded-lg bg-blue-600 px-2.5 py-1 text-[11px] font-black text-white shadow-xs">
                  <span className="flex items-center gap-1">
                    ...
                    ⭐ Top Đề Xuất: An Toàn & Tối Ưu Phí/Lãi
                  </span>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  <div
                    className="flex h-10 w-10 items-center justify-center rounded-xl text-white font-extrabold text-sm shadow-xs shrink-0"
                  ...
                  <button
                    ...
                    className={`rounded-lg p-1.5 text-xs font-semibold transition-colors shrink-0 ${
                      isCompared
              ...
              <div className="broker-card__body">
                ...
                {/* Metrics Grid */}
                <div className="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-2.5 dark:bg-slate-800/60 leading-normal">
                  {/* Trading fee metric */}
                  <div>
                ...
                {/* Promotion snippet */}
                <div className="mt-3 rounded-lg border-l-2 border-amber-400 bg-amber-50/40 p-2 dark:bg-amber-950/20 text-[11px]">
                  <div className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1">
                    <Sparkles className="h-3 w-3 text-amber-500" />
                    Ưu đãi nổi bật:
                  </div>
                  <p className="line-clamp-2 text-slate-500 dark:text-slate-400 pl-2.5 leading-relaxed text-[11px]">
              ...
              <footer className="broker-card__footer mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                <button
                ...
                <a
                  ...
                  className={`flex-1 inline-flex items-center justify-center gap-1 rounded-xl py-2 text-xs font-extrabold shadow-sm transition-all ${
                    isBsc
